...
class EntireDoublyCircularLinkList{
public:
Node *tail;
EntireDoublyCircularLinkList(){
    tail=NULL;
}
...
void PrependAtStart(int data){
    ...
    if(tail==NULL){
        ...
        new_node->next=tail->next;//node->next=head
        tail->next->prev=new_node;//head->prev=node
        tail->next=new_node;
        new_node->prev=tail;
    }
}
...
void InsertAtNode(int Id_of_node,int data){
    ...
    temp=temp->next;
    count_Node++;
    }while(temp!=tail->next && count_Node != Id_of_node);
...
int main(){
EntireDoublyCircularLinkList D1;
